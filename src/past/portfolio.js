(function($){
	...
	function _loadProjects(){
		...
		for(var i=0; i<_myProjects.length; i++){
				...
				// -- Project thumbs
				$("#_imageViewersContainer0").append('<div class="brick" data-id="_projectThumbnail' + String(i) + '" data-numb="'+String(i)+'" style="width:'+String((2 + (2 * Math.random() << 0))*100)+'px;  text-align:left; border-bottom:solid thin #000; opacity:0;"><img src="img/'+String(_myProjects[i].thumb)+'" width="100%"><p style="font-family:Raleway, Helvetica, sans-serif;">'+String(_myProjects[i].title)+'</p><p style="font-size:75%; font-family:open_sanslight">'+String(_myProjects[i].subtitle)+'</p></div>');
	...
	function _newStylize(){
		...
		$('.informationList' ).css({'font-size':'1.0rem', 'padding':'2px 0px', 'font-family':'Didot'});
	...
	function _loadAdditionalSlides(){
		...
		if(_pageType !== 'mkkpn' && _pageType !== false){
			...
			$('#_imageViewersContainer'+String(1)).append('<div class="_imageViewers"><div class="_imageViewers _parallax" style="margin-top:120px;"></div><div class="overlay"></div><div class="_imageTextBox"><div style="padding-bottom:20px;"><span style="font-family: Raleway, Helvetica, sans-serif; font-weight: lighter; color: #777; font-size:1.6rem;">Client List</span><p style="font-family: Raleway, Helvetica, sans-serif; font-weight: lighter; color: #777; font-size:1rem;">Following list is clients of my personal and of the organization where I belonged to</p></div><div style="width:100%; max-width:1280px; border-bottom:solid thin #CCC; border-top:solid thin #CCC; padding:50px 0px;"><div id="_clientListContainer"></div></div></div></div>');
			
			for(var i=0; i<_clientlist.length; i++){
				console.log(_clientlist[i]);
				$('#_clientListContainer').append('<span style="padding:20px;  display: inline-block; height: 100%; vertical-align: middle;"><img src='+_clientlist[i]+' style="width:160px; margin:20px;"/></div>');
			}
		}
...
               return false;
	...
	function _createSortButton(){
		...
		for(var i=0; i<_myProjectSort.length; i++){
			...
			if(i < _myProjectCategories.length){ 
				$("#_myProjectSort"+String(i)).css({'font-weight':'bold', 'font-family':'Raleway, Helvetica, sans-serif', 'font-size':'2.5rem', 'padding':'1rem 3rem'});
			}else if(i < (_myProjectCategories.length+_myProjectRoles.length)){
				$("#_myProjectSort"+String(i)).css({'font-style':'italic', 'font-family':'Didot, Helvetica, sans-serif', 'font-size':'2.5rem', 'padding':'1rem 3rem'});
			}else{
				$("#_myProjectSort"+String(i)).css({'font-style':'italic', 'font-weight':'bold', 'font-family':'Raleway, Helvetica, sans-serif', 'font-size':'1.5rem', 'padding':'1rem 1rem'});
			}
	...
	       _sortProject(_findCategNumber(location.hash.replace("#", "")));
	    }
	}
